<template>
  ...
  >
    <el-form :model="form" ref="formRef" label-width="80" :show-message="false">
      ...
      <el-form-item label="Username" prop="username" required :disabled="form.id">
...
<script setup>
import { addUser, putUser } from '@/services/user'
...
const handleConfirm = () => {
  formRef.value.validate(valid => {
    if (valid) {
      loading.value = true
      // IDB无法处理proxy对象
      ;(props.data?.id ? putUser : addUser)(toRaw(form.value))
        .then(() => {
          emit('success')
          emit('update:modelValue', false)
        })
