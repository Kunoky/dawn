  ...
  >
    <el-form :model="form" ref="formRef" label-width="120" :rules="rules" v-loading="dataLoading">
      <el-row>
        ...
        <el-col :span="12">
          <el-form-item v-if="props.data" label="设备型号" prop="modelNo" class="form_flex">
          ...
          <el-form-item v-else label="设备型号" prop="modelNo">
            <el-input v-model="form.modelNo" disabled placeholder="自动填入" />
          </el-form-item>
        </el-col>
        <el-col :span="12">
        ...
        <el-col :span="12">
          <el-form-item label="payer客户" prop="payerName" class="form_flex">
            ...
            <el-input
              ...
              style="width: 59%"
            />
            <el-button style="width: 10%" type="primary" @click="handleCustDesc1">
              {{ show1 ? '新建' : '搜索' }}
            </el-button>
          </el-form-item>
        </el-col>
...
const { run: getRequestInfo, loading: dataLoading } = useAsync(
  ...
  },
  {
    onSuccess(res) {
      ...
      staging.value.bbb = res.data.payerName
...
const changeSeriaNo = val => {
  ...
  if (val.payer.customerId === null || show1 === false) {
    form.value.isConsistentSap = false
  }
  show.value = true
  show1.value = true
  ...
  staging.value.bbb = val.payer.name === '' ? val.payer.enName : val.payer.name
}
...
const changeCustDesc1 = val => {
  ...
  form.value.payerName = val.name === '' ? val.enName : val.name
...
async function getEngineerName(v) {
  return req.get('/user/fse', { params: { fseName: v } }).then(res => {
    engineerNameList.value = res.data.map(item => {
      return { value: item, label: `${item.fseName}_${item.fseWorkCenter}` }
    })
  })
}
