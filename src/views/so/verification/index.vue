<template>
  <div>
    ...
    >
      ...
      <el-table-column label="操作" fixed="right" class-name="small-padding fixed-width" width="130">
        <template #default="{ row }">
          ...
          <el-button type="primary" link @click="handleExamine(row)">核验通过</el-button>
        </template>
      </el-table-column>
      <!-- <template #actions>
    ...
    <FormDialog :data="current" v-model="visible.detail" @success="handleFormSuccess"></FormDialog>
  </div>
</template>
...
const handleExamine = () => {
  ElMessageBox.confirm('是否确认核验通过', i18n.t('common.warning'), {
